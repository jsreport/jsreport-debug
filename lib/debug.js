  ...
  reporter.initializeListeners.add('request-log', () => {
    reporter.renderErrorListeners.add('request-log', (request, res, e) => {
      if (request.options.debug && request.options.debug.logsToResponse) {
        e.message += os.EOL + 'logs' + (request.options.isChildRequest ? ' (child request):' : ':') + os.EOL + logsToFormattedString(request.context.logs)
      }
    })
    ...
    reporter.afterRenderListeners.add('request-log', (request, response) => {
      ...
      request.context.logs.push({
        message: `Rendering request ${request.context.id} finished in ${(new Date().getTime() - request.context.startTimestamp)} ms`,
        timestamp: new Date().getTime(),
        level: 'info'
      })

      if (request.options.debug && request.options.debug.logsToResponseHeader && response.meta.headers) {
        response.meta.headers['Debug-Logs'] = JSON.stringify(cutResponseHeader(request.context.logs, reporter.options.debug.maxLogResponseHeaderSize, reporter.logger))
      }
